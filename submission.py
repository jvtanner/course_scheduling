...
def create_chain_csp(n):
    ...
    for index in range(len(variables)):
        ...
        if index > 0:
            csp.add_binary_factor(variables[index - 1], variables[index], lambda a, b: a != b)
    # END_YOUR_CODE
    return csp
...
def create_nqueens_csp(n = 8):
    ...
    for one in range(n):
        for two in range(n):
            if one != two:
                csp.add_binary_factor(vars[one], vars[two],
                                      lambda rowcol1, rowcol2: abs(rowcol1 - rowcol2) != abs(
                                          one - two))
...
class BacktrackingSearch():
    ...
    def get_unassigned_variable(self, assignment):
        ...
        if not self.mcv:
            ...
            smallestNumVars = float('inf')
            mostRestricted = self.csp.variables[0]
            for var in self.csp.variables:
    ...
    def arc_consistency_check(self, var):
        ...
        # adjustment = 0
        # queue = []
        # # Populate a queue with the variable's neighbors
        # for neighbor in self.csp.get_neighbor_vars(var):
        #     queue.append((neighbor, var))
        # # Check each neighbor variable to see if any values come out to ZERO when under partial assignment
        # while len(queue) != 0:
        #     neighborVar, currentVar = queue.pop(0)
        #     uselessValues = []
        #     nValues = tuple(self.domains[neighborVar])
        #     # For each neighbor value,
        #     for currentVal in self.domains[currentVar]:
        #
        #         # check the current var's values to see if combinations comes out to ZERO
        #         for neighborVal in nValues:
        #             if self.csp.binaryFactors[neighborVar][currentVar][neighborVal][currentVal] == 0:
        #                 listofremoved.append((neighborVal, neighborVar))
        #                 if neighborVal in self.domains[neighborVar]:
        #                     self.domains[neighborVar].remove(neighborVal)
        #                 for neighOfNeigh in self.csp.get_neighbor_vars(neighborVar):
        #                     if neighOfNeigh != neighborVar:
        #                         queue.append((neighOfNeigh, neighborVar))
        #         # Apply the same process to the neighbor's neighbors
            ...
            for neighborVal in self.domains[neighborVar]:
                ...
                for currentVal in self.domains[currentVar]:
                    if self.csp.binaryFactors[neighborVar][currentVar][neighborVal][currentVal] > 0:
                        usefulness = 1
                if usefulness == 0:
                    uselessQueue.append(neighborVal)
            # Get rid of values that equate to ZERO
            while len(uselessQueue) != 0:
                tossValue = uselessQueue.pop(0)
                    # listofremoved.append((neighborVal, neighborVar))
                self.domains[neighborVar].remove(tossValue)
                # Apply the same process to the neighbor's neighbors
                for neighOfNeigh in self.csp.get_neighbor_vars(neighborVar):
                    if neighOfNeigh != neighborVar:
                        queue.append((neighOfNeigh, neighborVar))
...
def get_sum_variable(csp, name, variables, maxSum):
    ...
    def basic(A_i, A_iPrev):
        return A_i[0] == A_iPrev[1]

    def consist(A_i, variable):
        return A_i[1] == (A_i[0] + variable)

    def first(A_i, variable):
        return A_i[1] == variable

    def last(result, A_i):
        return result == A_i[1]

    result = ('sum', name, 'final')
    if len(variables) == 0:
        ...
        for index, Xi in enumerate(variables):
            ...
            if index > 0:
                for j in domain:
                    for k in domain:
                        track.append((k, j))
                # Add A_i as a variable
                csp.add_variable(A_i, track)
                # Consistency
                csp.add_binary_factor(A_i, variables[index], consist)
                # Basic
                csp.add_binary_factor(A_i, A_iPrev, basic)
            else:
                for i in domain:
                    firstDomain.append((0, i))
                csp.add_variable(A_i, firstDomain)
                # Restriction on the first time
                csp.add_binary_factor(A_i, variables[0], first)

            A_iPrev = A_i
        # incorporate final result
        csp.add_variable(result, domain)
        # Constraint of final
        csp.add_binary_factor(result, A_i, last)
    return result
    # END_YOUR_CODE
